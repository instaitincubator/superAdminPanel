...
import { CrossedEye } from "@/shared/icons/crossedEye"
      ...
      <div className={cn(fullWidth ? "w-full" : "w-[240px]")}>
        {label && (
          <span className="text-regular-14 text-light-900">
            {label}
            {requiredElem && (
              <span className="text-red-500 font-bold ml-0.5 text-lg">*</span>
            )}
          </span>
        )}
        ...
        {error && (
          <span className="text-regular-14 text-danger-500">{error}</span>
        )}
      </div>
    )
  }
